...
static BROTLI_INLINE void FN(FindLongestMatch)(
  ...
  if (dictionary->use_transformed_dict == BROTLI_TRUE) {
    if (max_length >= 4 &&
        ...
        self->common_->dict_num_matches >=
        (self->common_->dict_num_lookups >> 7)) {
      SearchInReducedDictionary(dictionary, self->common_, &data[cur_ix_masked],
          max_length, dictionary_distance, max_distance, out, best_score);
    }
  }
  else if (min_score == out->score) {
